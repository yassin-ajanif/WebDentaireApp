<div class="pb-8 pl-0 pr-3 pt-2 sm:pr-4">
    <style>.qp-drop{display:none;position:absolute;z-index:9999;margin-top:2px;width:100%;border-radius:6px;border:1px solid #d1d5db;background:#fff;box-shadow:0 4px 12px rgba(0,0,0,.12);max-height:200px;overflow-y:auto}.qp-drop.show{display:block}.qp-item{cursor:pointer;padding:6px 10px;font-size:13px;color:#374151}.qp-item:hover,.qp-item.highlighted{background:#eff6ff;color:#1e3a8a}.qp-item small{color:#9ca3af;margin-left:6px}.queue-board-search{position:relative;width:10rem;max-width:100%;flex-shrink:0}.queue-board-search>.app-input{display:block;width:100%;box-sizing:border-box;padding:.375rem .5rem .375rem 2rem;font-size:.875rem;line-height:1.25}.queue-board-search>.queue-board-search-icon{position:absolute;left:.35rem;top:0;bottom:0;width:1.5rem;display:flex;align-items:center;justify-content:center;pointer-events:none;color:#9ca3af}.queue-board-search>.queue-board-search-icon svg{display:block;width:16px;height:16px;flex-shrink:0}.queue-section-details{border:1px solid var(--color-border-default);border-radius:0.5rem;background:rgb(217 217 217 / 0.3);overflow:hidden;margin-bottom:0.75rem}.queue-section-details>summary{cursor:pointer;list-style:none;padding:0.65rem 1rem;font-weight:600;color:var(--color-text-primary);display:flex;align-items:center;justify-content:space-between;gap:0.5rem;-webkit-user-select:none;user-select:none}.queue-section-details>summary::-webkit-details-marker{display:none}.queue-section-details>summary::marker{content:''}.queue-section-details>summary::after{content:'▾';flex-shrink:0;opacity:0.55;font-size:0.7rem;line-height:1}.queue-section-details[open]>summary{border-bottom:1px solid var(--brush-stroke-gray)}.queue-section-details[open]>summary::after{content:'▴'}.queue-section-details .queue-section-body{padding:0.75rem 1rem 1rem}</style>
    <script>
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex shrink-0 items-center gap-2">
            <div class="queue-board-search">
                <input type="text" wire:model.live="search" placeholder="{{ __('Rechercher...') }}"
                       class="app-input">
                <span class="queue-board-search-icon" aria-hidden="true">
    ...
    <div class="space-y-2">
        <details class="queue-section-details">
            <summary>
                <span>{{ __('queue.section.finished', ['count' => $done->count()]) }}</span>
            </summary>
        ...
        <details class="queue-section-details" open>
            <summary>
                <span>{{ __('queue.section.active', ['count' => $waiting->count()]) }}</span>
            </summary>
            <div class="queue-section-body">
                <div class="flex gap-4">
                    <div class="w-2 shrink-0 rounded-full" style="background-color: var(--color-accent-success);"></div>
                    <div class="grid min-w-0 flex-1 grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        <details class="queue-section-details">
            <summary>
                <span>{{ __('queue.section.cancelled', ['count' => $cancelled->count()]) }}</span>
            </summary>
            <div class="queue-section-body">
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
